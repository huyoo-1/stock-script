...
- 每天收盘汇总时把全 A 收盘价快照保存下来（`data/price_history/` 按交易日存，只保留最近约一年），供未来做技术面分析，不用额外请求外部 K 线接口
- 新增筛选逻辑：**连续 3 日上涨 且 现价 ≥ 5 日线 > 10 日线**
- 筛选结果只在收盘卡片里汇总一个**总数**，明细等以后 Web 面板做出来再看
- 数据需要积累约 10 个交易日后 MA10 才有意义，之前卡片会显示"数据积累中（N/10 个交易日）"
- 新增本地验证脚本 `tests/test_price_screener.js`（不联网，跑完自动清理）
