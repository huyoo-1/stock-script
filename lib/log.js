...
function createLogger(logDir) {
  ...
  function write(level, stream, file, msg, extra) {
    stream = rotate(stream, file);
    ...
    if (level === 'ERROR') {
      errStream = rotate(errStream, errPath);
  ...
  return {
    info: (msg, extra) => write('INFO', runStream, runPath, msg, extra),
    warn: (msg, extra) => write('WARN', runStream, runPath, msg, extra),
    error: (msg, extra) => write('ERROR', runStream, runPath, msg, extra),
  };
}
