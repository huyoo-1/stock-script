...
// 新浪返回 GBK 编码，需用 TextDecoder('gbk') 解码
async function fetchSinaQuote(http, code, exchange, kind) {
  const prefix = (exchange === 'SH' ? 'sh' : 'sz') + code;
  let buffer;
  try {
    const res = await axios.get(SINA_HQ_URL + prefix, {
      headers: { Referer: SINA_REFERER, 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
