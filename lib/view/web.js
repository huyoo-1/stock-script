...
// 静态：/ 伺服 web/dist/index.html（Vite 构建产物），/vendor/* 伺服本地 ECharts。
...
// 当天无盘中数据时回退到 intraday.json 里最近的日期，便于回看历史数据
function apiIntraday(res, url) {
  const q = url.searchParams.get('date');
  const todayStr = calendar.dateStr(new Date());
  let dateStr = !q || q === 'today' ? todayStr : q;
  let records = _history.getDayIntraday(dateStr);
  if (records.length === 0 && (!q || q === 'today')) {
    const all = _history.readIntraday();
    if (all.length > 0) {
      const latest = all.slice().sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time))[0];
      dateStr = latest.date;
      records = _history.getDayIntraday(dateStr);
    }
  }
...
// 静态文件伺服：/vendor/* 从 web/vendor，其余从 web/dist（Vite 构建产物）
function serveStatic(req, res, pathname) {
  // /vendor/* 仍由本地伺服（阶段一 echarts.min.js）
  if (pathname.startsWith('/vendor/')) {
    return serveFile(res, pathname.replace(/^\/vendor\/+/, ''), VENDOR_DIR, { allowDir: false });
  }
  // dist 缺失守卫：未构建时返回 503 提示
  const indexPath = path.join(DIST_DIR, 'index.html');
  if (!fs.existsSync(indexPath)) {
    res.writeHead(503, { 'Content-Type': 'text/html; charset=utf-8' });
    return res.end('<meta charset="utf-8"><h3>前端未构建</h3><p>请先运行 <code>npm run build</code> 或 <code>npm start</code>（prestart 钩子会自动构建）。</p>');
  }
  const rel = pathname === '/' ? 'index.html' : pathname.replace(/^\/+/, '');
  // SPA fallback：无扩展名且非 assets/ → 回退 index.html（客户端路由刷新可存活）
  const hasExt = path.extname(rel) !== '';
  return serveFile(res, rel, DIST_DIR, {
    fallback: !hasExt ? 'index.html' : null,
    allowDir: false,
  });
}

// 通用文件伺服（防目录穿越 + 缓存头 + SPA fallback）
function serveFile(res, rel, baseDir, { fallback, allowDir } = {}) {
  const filePath = path.resolve(baseDir, rel);
  if (filePath !== baseDir && !filePath.startsWith(baseDir + path.sep)) {
    return json(res, 403, { error: '禁止访问' });
  }
  if (fs.existsSync(filePath) && (allowDir || !fs.statSync(filePath).isDirectory())) {
    const ext = path.extname(filePath).toLowerCase();
    const isAsset = rel.startsWith('assets/');
    const cache = ext === '.html' ? 'no-store'
      : isAsset && ['.js', '.mjs', '.css', '.woff2', '.woff', '.ttf'].includes(ext)
  ...
  if (fallback) {
    const fbPath = path.resolve(baseDir, fallback);
    if (fs.existsSync(fbPath)) {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
      return fs.createReadStream(fbPath).pipe(res);
    }
  }
  return json(res, 404, { error: '文件不存在' });
}

module.exports = { createWebServer, WEB_DIR, DIST_DIR };
