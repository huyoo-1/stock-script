...
async function fetchMargin(http, logger) {
  ...
  try {
    ...
    if (sse && szse) {
      // 两市都成功：合计（上交所元→亿元，深交所本就是亿元）
      const fin = sse.fin + szse.fin;
      const loan = sse.loan + szse.loan;
      const total = Math.round((fin + loan) * 100) / 100;
      logger && logger.info(`融资融券(交易所): 沪${sse.fin} 深${szse.fin} 合计${total}`);
      return {
    ...
    // 只拿到一边：不把缺失的另一边当 0 混入合计，返回单市数据并标注 partial
    const only = sse || szse;
    if (only) {
      const which = sse ? '沪市' : '深市';
      logger && logger.warn(`融资融券备选仅${which}可用，合计仅含${which}`);
      return {
