...
async function runClose({ config, http, feishu, logger, intradayCache }) {
  ...
  // 5. 全 A 收盘价快照落盘 + 技术筛选（结果仅汇总总数推送，明细供 Web 面板查看）
  ...
  const screenResults = screener.runScreener(recent);
  const screenSummary = {
    count: screenResults.length,
    readyDays: screener.readyDays(recent),
    neededDays: screener.MA10_DAYS,
  };
  logger.info(`技术筛选：${screenSummary.count} 只（数据 ${screenSummary.readyDays}/${screenSummary.neededDays} 个交易日）`);
