...
async function runIntraday({ config, http, feishu, logger, intradayCache, time }) {
  ...
  // 7. 建卡推送（与调度解耦：不 await，推送重试不阻塞下一次调度触发）
  ...
  // 落盘（含 quote + tip，供 Web 面板展示当日盘中轨迹；重启不丢）
  try {
...
// 收盘汇总
async function runClose({ config, http, feishu, logger, intradayCache, ma20Provider }) {
  const today = todayStr();
  logger.info(`=== 收盘汇总 ${today} ===`);
  // MA20 数据提供者（service 注入，独立脚本未传时自建）
  const ma20 = ma20Provider || createMa20Provider({ http, config, logger });
  ...
  // 5. 全 A 收盘价快照落盘 + 技术筛选（本地 10 日粗筛 → MA20 二次精筛，明细供 Web 面板查看）
  ...
  // 推送与调度解耦：不 await，避免推送重试（5s/15s/30s）阻塞下一次调度
