...
// fetchQuoteFn: 可选，策略模式注入的 ETF 行情获取函数 (code) => Promise<quote|null>
async function fetchEtfFlows(http, whitelist, prevClose, surgeRatio, logger, fetchQuoteFn) {
  const prevMap = {};
  ...
  for (const code of whitelist) {
    try {
      const q = fetchQuoteFn ? await fetchQuoteFn(code) : await em.fetchEtfQuote(http, code);
      if (!q) {
